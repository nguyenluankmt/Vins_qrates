...
(function (window, document) {
	...
	Croppic.prototype = {
		...
		afterCrop: function (data) {
            ...
            if (response.status == 'success') {
                if(response.dealer == 1) {
                    $('#dealership-image').html(response.url);
                } else {
                    if(response.edit == 'true') {
                    	$('#' + response.parent_id).empty();
                        $('#' + response.parent_id).replaceWith(response.url);
                    } else {
                        $('#' + that.id).parent().parent().prepend(response.url);
                    }
                }
...
//                that.croppedImg = that.obj.find('.croppedImg');
                that.init();
                that.hideLoader();
                $("input#something_image").val(1);
                $("label#something_image-error").hide();
                $(".bo-gallery-upload").removeAttr('style'); 
                $(".bo-img-container").removeAttr('style'); 
				$(".vehicle .bo-gallery-block .img-thumbnail").each(function(){
				    $(this).attr('src', $(this).attr('src') + '?now=' + Date.now());
					var url = $(this).attr("src");
					// var url = "http://motory.local/wp-content/uploads/2015/07/large/vehicle-0-1437452865.jpegaa";
					var $this = $(this);
					// console.log(url);
